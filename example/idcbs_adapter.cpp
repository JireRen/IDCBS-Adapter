...
int main(int argc, char** argv)
{
    ...
    desc.add_options()
            ...
            ("map,m", po::value<std::string>(&inputFile)->required(), "path to the input .map file")
            // ("agents,a", po::value<std::string>()->required(), "path to the input .agents or .scen file")
            ("output,o", po::value<std::string>(&outputFile)->required(), "path to the .csv output file")
    ...
    // read the YAML
    YAML::Node agent_config = YAML::LoadFile(inputFile);

    std::vector<std::pair<int, int> > obstacles;
    std::vector<std::pair<int, int> > starts;
    std::vector<std::pair<int, int> > goals;

    std::string mapFile = agent_config["map_path"].as<std::string>();
    ...
    for (const auto& node : agent_config["agents"]) {
    ...
    MapLoader ml(dimx, dimy, obstacles);
